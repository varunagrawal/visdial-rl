...
def dialogDump(
    params, dataset, split, aBot, qBot=None, beamSize=1, savePath="dialog_results.json"
):
    """
    ...
    """
    assert aBot is not None or (
        qBot is not None and aBot is not None
    ...
    to_str_gt = lambda w: str(
        " ".join([ind2word[x] for x in filter(lambda x: x > 0, w.data.cpu().numpy())])
    )  # .encode('utf-8','ignore')
    to_str_pred = lambda w, l: str(
        " ".join(
            [ind2word[x] for x in list(filter(lambda x: x > 0, w.data.cpu().numpy()))][
                : l.data.cpu()[0]
            ]
        )
    )  # .encode('utf-8','ignore')
    ...
    if "%s_img_fnames" % split not in dataset.data.keys():
        print(
            "[Error] Need coco directory and info as input "
    ...
    for idx, batch in enumerate(dataloader):
        ...
        for round in range(numRounds):
            if aBot is not None and qBot is None:
                ...
                aBot.observe(
                    round, ans=gtAnswers[:, round], ansLens=gtAnsLens[:, round]
                )
                _ = aBot.forward()
            ...
            for j in range(batchSize):
                question_str = (
                    to_str_pred(questions[j], quesLens[j])
                    if qBot is not None
                    else to_str_gt(gtQuestions[j])
                )
                answer_str = to_str_pred(answers[j], ansLens[j])

                dialog[j]["dialog"].append(
                    {"answer": answer_str[8:], "question": question_str[8:] + " "}

...
def encode_vocab(data_toks, ques_toks, ans_toks, word2ind):
    ...
    for k, v in data_toks.items():
        image_id = k
        caption = [word2ind.get(word, word2ind["UNK"]) for word in v["caption"]]
    ...
    for i in ans_toks:
        answer = [word2ind.get(word, word2ind["UNK"]) for word in i]
...
def create_data_mats(data_toks, ques_inds, ans_inds, params, dtype):
    ...
    for i in range(num_threads):
        ...
        if dtype == "test":
            ...
            image_list.append(
                "%s2014/COCO_%s2014_%012d.jpg" % ("train", "train", image_id)
            )
        image_index[i] = i
        caption_len[i] = len(data_toks[image_id]["caption_inds"][0:max_cap_len])
        captions[i][0 : caption_len[i]] = data_toks[image_id]["caption_inds"][
            0:max_cap_len
        ]
        ...
        for j in range(num_rounds):
            if data_toks[image_id]["dialog"][j]["question"] != -1:
                question_len[i][j] = len(
                    ques_inds[data_toks[image_id]["dialog"][j]["question"]][
                        0:max_ques_len
                    ]
                )
                questions[i][j][0 : question_len[i][j]] = ques_inds[
                    data_toks[image_id]["dialog"][j]["question"]
                ][0:max_ques_len]
            if data_toks[image_id]["dialog"][j]["answer"] != -1:
                answer_len[i][j] = len(
                    ans_inds[data_toks[image_id]["dialog"][j]["answer"]][0:max_ans_len]
                )
                answers[i][j][0 : answer_len[i][j]] = ans_inds[
                    data_toks[image_id]["dialog"][j]["answer"]
                ][0:max_ans_len]
    ...
    for i in range(num_threads):
        ...
        for j in range(num_rounds):
            options[i][j] = (
                np.array(data_toks[image_id]["dialog"][j]["answer_options"]) + 1
            )
...
def map_format(data, split):
    ...
    for ix, dp in enumerate(data):
        ...
        dialogs.append({"image_id": imid, "caption": cap, "dialog": dialog})

    mapped_data = {
        "data": {"questions": questions, "answers": answers, "dialogs": dialogs},
        "split": split,
        "version": "0.5",
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-download", default=0, type=int, help="Whether to download VisDial v0.9 data"
    )
    parser.add_argument(
        "-version",
        default="0.5",
        help="Choose the dataset version: 0.5 | 0.9",
        choices=["0.5", "0.9"],
    )
    ...
    parser.add_argument(
        "-input_json_train",
        default="visdial_0.5_train.json",
    ...
    parser.add_argument(
        "-output_json",
        default="visdial/chat_processed_params.json",
    ...
    # Options
    parser.add_argument(
        "-max_ques_len", default=20, type=int, help="Max length of questions"
    )
    parser.add_argument(
        "-max_ans_len", default=20, type=int, help="Max length of answers"
    )
    parser.add_argument(
        "-max_cap_len", default=40, type=int, help="Max length of captions"
    )
    parser.add_argument(
    ...
    assert (args.version in args.input_json_train) and (
        args.version in args.input_json_val
    ...
    if args.version == "0.5":
        if args.download == 1:
            os.system(
                "wget https://computing.ece.vt.edu/~abhshkdz/data/visdial/visdial_0.5_train.json"
            )
            os.system(
                "wget https://computing.ece.vt.edu/~abhshkdz/data/visdial/visdial_0.5_val.json"
            )
            os.system(
                "wget https://computing.ece.vt.edu/~abhshkdz/data/visdial/visdial_0.5_test.json"
            )
